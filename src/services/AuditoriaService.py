...
class AuditoriaService:
    @staticmethod
    def registrar_acao(db: Session, funcionario_id: int, acao: str, recurso: str, recurso_id: Optional[int] = None, dados_antigos: Optional[Dict[str, Any]] = None, dados_novos: Optional[Dict[str, Any]] = None, request: Optional[Request] = None) -> bool:
        try:
            ...
            if dados_antigos:
                if hasattr(dados_antigos, "__dict__"):
                    # É um objeto SQLAlchemy, converter para dicionário
                    dados_antigos_dict = {column.name: getattr(dados_antigos, column.name)
    ...
    @staticmethod
    async def registrar_acao_async(db, funcionario_id: int, acao: str, recurso: str, recurso_id: Optional[int] = None, dados_antigos: Optional[Dict[str, Any]] = None, dados_novos: Optional[Dict[str, Any]] = None, request: Optional[Request] = None) -> bool:
        try:
            ip_address = None
            user_agent = None
            ...
            if dados_novos:
                if hasattr(dados_novos, "__dict__"):
                    dados_novos_dict = {column.name: getattr(dados_novos, column.name) for column in dados_novos.__table__.columns}
                    dados_novos_json = json.dumps(dados_novos_dict, default=str)
                else:
                    dados_novos_json = json.dumps(dados_novos, default=str)
            else:
            ...
            if dados_antigos:
                if hasattr(dados_antigos, "__dict__"):
                    dados_antigos_dict = {column.name: getattr(dados_antigos, column.name) for column in dados_antigos.__table__.columns}
                    dados_antigos_json = json.dumps(dados_antigos_dict, default=str)
                else:
                    dados_antigos_json = json.dumps(dados_antigos, default=str)
            else:
